#!/usr/bin/env python3
"""
MAPF adversarial visualization for Akash's PACT experiments.
...
• **FGSM observation attacks** (invisible) perturb each social agent's sensor
    reading to stress-test observation robustness.
...
──────────────
  ...
  2. baseline_attacked.gif       — Baseline policy  + adversary chasers + FGSM obs noise
    3. pact_clean.gif              — PACT policy, no adversary
    4. pact_attacked.gif           — PACT policy      + adversary chasers + FGSM obs noise
    5. comparison_attacked.gif     — side-by-side  (baseline vs PACT)  under attack
"""
...
except ImportError:  # Support direct execution: python pact/visualize.py
    from path_utils import PACT_DIR, configure_paths, shared_path
...
def run_episode(social_policy, n_social, n_adversary,
    ...
    adversary_strategy: random_walk, astar_pursuit, goal_blocking, or mixed.
    """
    ...
    adv_rng = np.random.default_rng(seed + 777)
...
def main():
    ...
    pa.add_argument(
        "--baseline-model",
        default=str(shared_path("models", "phase2_smoke_baseline", "best_policy.pt")),
    )
    pa.add_argument(
        "--robust-model",
        default=str(PACT_DIR / "models" / "main_akash_curriculum" / "final_policy.pt"),
    )
    pa.add_argument("--baseline-label", default="Baseline Policy")
    pa.add_argument("--robust-label", default="PACT Policy")
    pa.add_argument("--output-dir", default=str(PACT_DIR / "results" / "animations"))
    ...
    pa.add_argument("--density",     type=float, default=0.3)
    ...
    pa.add_argument("--physical-only", action="store_true",
                    help="Disable FGSM perturbation in attacked GIFs and show only physical chasers")
    pa.add_argument("--adversary-strategy", default="astar_pursuit",
    ...
    print(
        ...
        f"strategy={args.adversary_strategy})…"
    )

    attack_fn = None if args.physical_only else fgsm_attack
    atk_kw = {} if args.physical_only else {"epsilon": args.fgsm_epsilon}
    attack_label = "chasers" if args.physical_only else f"chasers + FGSM (ε={args.fgsm_epsilon})"
    title_attack_label = (
        f"{args.num_adversary} Chasers"
        if args.physical_only
        else f"{args.num_adversary} Chasers + FGSM ε={args.fgsm_epsilon}"
    )

    print(f"  1/4  {args.baseline_label} — no adversary")
    ep_bc = run_episode(base, args.num_social, 0, **common)
    print(f"  2/4  {args.baseline_label} — {attack_label}")
    ep_ba = run_episode(base, args.num_social, args.num_adversary,
                        attack_fn=attack_fn, attack_kwargs=atk_kw,
                        adversary_strategy=args.adversary_strategy, **common)
    print(f"  3/4  {args.robust_label} — no adversary")
    ep_fc = run_episode(pact, args.num_social, 0, **common)
    print(f"  4/4  {args.robust_label} — {attack_label}")
    ep_fa = run_episode(pact, args.num_social, args.num_adversary,
                        attack_fn=attack_fn, attack_kwargs=atk_kw,
                        adversary_strategy=args.adversary_strategy, **common)

    for tag, ep in [(f"{args.baseline_label} clean", ep_bc),
                    (f"{args.baseline_label} attacked", ep_ba),
                    (f"{args.robust_label} clean", ep_fc),
                    (f"{args.robust_label} attacked", ep_fa)]:
        last = ep[-1]
    ...
    snaps_to_gif(ep_bc, f"{args.output_dir}/baseline_clean.gif",
                 f"{args.baseline_label} — No Adversary", args.fps)
    snaps_to_gif(ep_ba, f"{args.output_dir}/baseline_attacked.gif",
                 f"{args.baseline_label} — {title_attack_label}", args.fps)
    snaps_to_gif(ep_fc, f"{args.output_dir}/pact_clean.gif",
                 f"{args.robust_label} — No Adversary", args.fps)
    snaps_to_gif(ep_fa, f"{args.output_dir}/pact_attacked.gif",
                 f"{args.robust_label} — {title_attack_label}", args.fps)

    print("\nGenerating side‑by‑side comparisons …")
    side_by_side_gif(
        copy.deepcopy(ep_ba), copy.deepcopy(ep_fa),
        f"{args.baseline_label} + {title_attack_label}",
        f"{args.robust_label} + {title_attack_label}",
